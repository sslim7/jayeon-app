...
전화번호는 앞자리 0이 사라지지 않도록 텍스트 셀로 저장한다. 파일 최대 2MiB이며 행 수는 제한하지 않는다(서버가 나누어 저장한다).
미리보기 목록에는 제외된 행을 모두 보여 주고 추가될 행은 앞에서 50명까지만 그린다. 나머지는 접어 두며 확정하면 모두 저장된다.
...
통화분석(서버 처리) 계약·업로드 구현·검증 안내: [docs/call-analysis.md](docs/call-analysis.md)
