...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    /*
     * 🔴 **폰 받아쓰기 화면도 같은 면제가 필요하다.** 등록 시트가 여기로 밀어 넣는데, 이
     * 줄이 없으면 화면은 **열리자마자 웹뷰로 튕긴다.** 그러면 `asr: "client"` 로 등록된
     * 통화는 아무도 받아쓰지 않은 채 6시간 뒤 실패로 정리되고, 사용자가 보는 증상은
     * 「등록했는데 문자 화면이 떴다」라서 원인이 **되돌림**이라는 것이 전혀 드러나지 않는다
     * (→ `app/asr-run.tsx`).
     */
    if (pathname === '/asr-run') return;
    /*
     * 🔴 **네이티브 등록 화면도 같은 면제가 필요하다.** 웹 등록 시트로는 폰 받아쓰기를 할 수
     * 없어서(웹뷰에서 고른 파일은 껍데기로 넘어가지 않는다 → `app/call-create.tsx`) 웹이
     * 이 화면을 열어 달라고 말하는데, 이 줄이 없으면 화면은 **열리자마자 웹뷰로 튕긴다.**
     * 증상은 「메뉴를 눌러도 문자 보내기가 뜬다」라서 원인이 이동이 아니라 **되돌림**이라는
     * 것이 화면에 전혀 드러나지 않는다.
     */
        ...
        */}
        ...
        <Stack.Protected guard={authed}>
          ...
          */}
          ...
          {/*
            로컬 받아쓰기 설정. 기기 관리와 같은 자리다 — 설정 안의 한 줄에서만 열리지만
            라우트는 설정과 형제다(→ 위 `devices` 주석의 같은 이유).

            🔴 **이 화면은 네이티브에서만 뜻이 있다.** whisper 는 웹에 없어서, 설정의 그 줄은
            웹에서 아예 서지 않는다(→ `hooks/use-asr-summary.ts`). 라우트 자체는 양쪽에
            두는데, 주소로 바로 열었을 때 화면이 「앱에서만 됩니다」를 말해 주어야 하기
            때문이다 — 없는 라우트로 두면 같은 상황이 not-found 로 보인다.
          */}
          <Stack.Screen name="asr-setup" />
          {/*
          ...
          */}
          ...
          {/*
            폰 받아쓰기 진행 화면. 통화 등록에서 「로컬 받아쓰기」를 켜면 여기로 들어온다.

            🔴 **이 화면이 끝까지 가야 그 통화가 완성된다** — 서버는 받아쓰기를 하지 않고
            기다리다가 6시간 뒤 실패로 정리한다(→ `components/call-asr-run.tsx`). 그래서
            껍데기 모드의 `/shell` 되돌림에서도 면제한다(위 효과의 같은 이유).

            ⚠️ 라우트는 웹에도 둔다. 주소로 열었을 때 「앱에서만 됩니다」를 말해 주어야 한다.
          */}
          <Stack.Screen name="asr-run" />
          {/*
            네이티브 통화분석 등록. 껍데기 모드에서는 웹 등록 시트가 「폰에서 받아쓰기로
            등록」으로 이 화면을 연다(→ `components/call-create.tsx`).

            🔴 **파일 선택이 네이티브여야 해서 있는 화면이다.** 웹뷰 안에서 고른 파일은
            껍데기로 넘길 길이 없다(→ `app/call-create.tsx`). 위 `/shell` 되돌림에서
            면제한 것도 같은 이유다 — 없으면 열리자마자 웹뷰로 튕긴다.
          */}
          <Stack.Screen name="call-create" />
        </Stack.Protected>
