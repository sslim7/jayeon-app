...
- [llama.rn 0.12.3](https://github.com/mybigday/llama.rn): Qwen3-0.6B Q8_0, 4,096 context, `n_batch` 512, temperature 0, **grammar 없음**(줄 단위 텍스트 출력). CPU 스레드는 코어 수의 절반(2~4), iOS Metal 99 layers. 더 큰 모델을 자동 선택하지 않는다.
...
발화/문장 경계 기준 2,000자 chunk를 생성하고 선택 모델 tokenizer로 `n_ctx - n_predict - 256` token 이하인지 확인한다. 경계를 찾을 수 없는 지나치게 긴 발화는 조용히 자르지 않고 분석 실패로 처리한다. chunk 결과(`chunk2:N`)와 요약 통합 단계(`merge:level:index`) 각각을 저장한다. 할 일/결정사항은 구조적으로 합치고 완전 동일 중복만 제거한다. 의미가 비슷한 서로 다른 표현은 유지한다. 상대 날짜로 실제 날짜를 추측하지 않고 null을 사용한다.

## 분석량과 속도

🔴 **이 절의 숫자가 곧 사용자의 대기 시간이다.** 갤럭시 S21에서 chunk마다 요약·상세·할 일·결정·상담 분석을 한 번에 JSON으로 만들게 두었더니(chunk당 최대 2,300 token, 초당 2~3 token) 짧은 샘플 통화가 **25분 뒤 실패**했다. 그래서 생성량을 줄이는 쪽으로 뒤집었다.

- chunk에서 만드는 것은 **요약·할 일(최대 3)·결정사항(최대 2)** 뿐이다. `n_predict`는 400이다.
- **상세 내용과 상담 분석은 기기에서 만들지 않는다.** 서버 계약(`internal/calls/model.go`)은 이 필드들이 `null`이면 400으로 거부하지만 **빈 배열은 받는다**(Go는 `[]`를 non-nil 빈 slice로 읽고, 검사는 `== nil`과 개수 상한뿐이다). 서버를 고치지 않고 클라이언트가 빈 배열을 채워 보낸다. 화면은 그 탭을 빈 채로 두지 않고 「이 버전의 기기 분석에서 만들지 않습니다 … 서버 분석이 준비되면 제공할 예정입니다」를 안내한다. 담당자·기한도 뽑지 않고 null이다(내용 줄에 그대로 남는다).
- 출력은 **JSON이 아니라 줄 단위 텍스트**(`요약:`, `할일: … | 근거: …`, `결정:`)다. JSON schema grammar는 151k vocab을 token마다 훑어 샘플링을 느리게 만들고, 작은 모델이 배열을 이어 붙이다 출력 한도에 닿으면 **구간 전체를 잃는다.** 줄 단위 출력은 중간에 잘려도 앞줄이 살아남아, 한도 도달(`stopped_limit`)이 더 이상 실패가 아니다 — 잘린 마지막 줄만 버린다. 줄 순서가 곧 우선순위라 잘리면 뒤에서부터(결정 → 할 일) 사라진다.
- 요약 통합은 **4개씩 묶어 한 번에** 합친다. 둘씩 짝지으면 구간 n개에 통합이 `n-1`번 필요한데, 4개 묶음이면 구간 10개가 통합 4번으로 끝난다. 구간이 하나면 통합이 없어 **호출 한 번으로 분석이 끝난다.**
- `n_ctx`는 8,192에서 4,096으로 내렸다. Qwen3-0.6B의 KV 캐시는 token당 약 112KiB라 이것만으로 약 448MiB를 돌려받는다.

`node scripts/call-analysis-estimate.cjs`가 위 설정값을 그대로 읽어 예상 소요 시간을 계산한다(실기기 없이 도는 **산수**이므로 측정값이 아니다). 생성 속도·프롬프트 처리 속도는 `--decode`, `--prefill`로 넘긴다 — 실기기 진단 한 줄(`AI 호출 N회 · 생성 N 토큰 · N 토큰/초`)에서 잰 값을 넣어야 뜻이 있다.
...
실패는 **정해진 코드로만** 이유를 남긴다(`call-errors.ts`). 네이티브 예외 문구에는 경로가, 파싱 오류에는 통화 원문 조각이 섞일 수 있어 원문을 그대로 저장·표시하지 않는다. 화면에는 「무엇이 실패했는지 + 이유 + (코드: X)」가 남고, LLM 진단 숫자(호출 수, 생성 토큰, 토큰/초, 출력 한도 도달 횟수, 요약만 건진 구간 수)도 함께 저장한다. 분석이 없는 통화의 요약·상세·할 일·상담 분석 탭에는 왜 비어 있는지(진행 중 단계 또는 실패 이유)를 안내한다.

chunk 출력을 읽지 못하면(요약 줄이 없으면) **요약만 다시 요청**한다 — 가장 중요한 것부터 건지고 그 구간의 할 일·결정사항은 포기한다. 그래도 안 되면 그 구간을 빼고 나머지로 결과를 만든다. 빠진 구간 수와 요약만 건진 구간 수는 숨기지 않고 목록과 상세에 표시한다. 한 구간도 남지 않으면 지금처럼 실패로 끝난다. 요약 통합이 실패하면 묶음의 요약들을 이어 붙인다(없는 내용을 만들지 않는다).

## 검증

`npm run test:calls`는 통화일시 파싱·기본값·미래 거절, 단계별 진행률·경과 시간, 실패 코드 화이트리스트와 스레드 수 결정, 부분 성공(구간 건너뛰기), 줄 단위 출력 파싱(라벨 흔들림·`<think>`·잘린 마지막 줄·근거 없는 할 일)과 요약만 받는 재시도, 통합 묶음 계산, 만들지 않는 항목의 빈 배열과 서버 개수 상한, grammar 없이 도는 생성 설정, JSON 계약/날짜/길이/중복 제거, 업로드 전 원문 정리(공백 세그먼트·비감소 start·상한), 업로드만 재시도, LLM 실패 후 STT 재사용, 분석 중 계정전환 차단, 400의 영구 실패 분류와 재분석 경로, 401의 재시도 유지, 업로드 백오프, contact 화이트리스트, 기기 시계 여유, 앱 재시작 후 `ANALYZING` 재개, 완료 후 정리를 검증한다. 타입 검사와 웹 빌드 검증을 별도로 수행한다. 실제 모델 다운로드·실기기 inference·한국어 평가 데이터 품질·RAM/열/배터리·장시간 실제 오디오·iOS 전체 native 빌드는 실기기 출시 전 확인해야 한다. 테스트용 가짜 분석 결과를 운영 코드에서 반환하지 않는다.
